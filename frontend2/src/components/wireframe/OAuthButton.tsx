...
export function OAuthButton({ provider, onLogin, className }: OAuthButtonProps) {
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={cn(
        'flex items-center justify-center gap-3 w-full px-4 py-3.5 rounded-lg font-medium text-sm transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed',
        isGoogle
    ...
    >
      {isLoading ? (
        ...
        <>
          ...
          {isLoading ? 'Signing in...' : `Continue with ${provider === 'google' ? 'Google' : 'GitHub'}`}
        </>
      )}
    </button>
  )
